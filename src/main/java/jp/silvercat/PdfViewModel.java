...
public class PdfViewModel implements IModel {
	...
	/** 最後に開いたPDFファイルのパス */
	protected File lastOpenFilePath_ = null;
		...
		ex.execute(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					model = new PdfFileModel(file, "");
					// Modelに監視役のリスナー（callback)を登録
					model.addModelListener(ml);
					model.load();
				} catch (InvalidPasswordException e) {
					model.removeModelListner(ml);
					model = null;
					String input = JOptionPane.showInputDialog(null, "文書を開くパスワードが設定されています。パスワードを入力してください。");
					if (input != null) {
						try {
							model = new PdfFileModel(file, input);
							// Modelに監視役のリスナー（callback)を登録
							model.addModelListener(ml);
							model.load();
	...
	protected class FileOpenHandler extends AbstractAction {
		...
		public synchronized void actionPerformed(ActionEvent event) {
			...
			if (self_.lastOpenFilePath_ == null) {
				FileSystemView fileSystemView = FileSystemView.getFileSystemView();
				self_.lastOpenFilePath_ = fileSystemView.getDefaultDirectory();
			}

			JFileChooser filechooser = new JFileChooser(self_.lastOpenFilePath_);
			...
			if (selected == JFileChooser.APPROVE_OPTION) {
				...
				for (File file : files) {
					...
					if (ext.equals(".PDF")) {
						...
						self_.lastOpenFilePath_ = new File(file.getParent());
					}
				}
			}
		}
		...
		public void actionPerformed(ActionEvent event) {
			...
			ex.execute(new Runnable() {
				...
				public void run() {
					...
					PdfFileModel newPdfFile;
					try {
						newPdfFile = new PdfFileModel(newFile, "");
					} catch (InvalidPasswordException e1) {
						throw new RuntimeException(e1);
					}
					...
					try {
						...
						newPdfFile.addModelListener(new IModelListener() {
							...
							public void modelChanged(ModelEvent event) {
								...
								if (processPage >= pagesSize || model.getStatus()
